...
const { isOpen, data, close } = useDialogManager<AttendanceDay>('delete-attendance-day');
</script>
            ...
            <div class="flex flex-col gap-2 max-sm:items-center sm:flex-row sm:gap-4">
                ...
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        ...
                        attendance day record for
                        <span v-if="data" class="font-medium">
                            {{ new Date(data.work_date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) }}
                        </span>
                        and remove all associated logs from the database.
                    </AlertDialogDescription>
                </AlertDialogHeader>
            </div>
            <AlertDialogFooter>
                ...
                <AlertDialogAction asChild>
                    <Form
                        v-if="data"
                        :action="AttendanceDayController.destroy(data.id)"
                        method="delete"
                        :onSuccess="() => close()"
                    >
